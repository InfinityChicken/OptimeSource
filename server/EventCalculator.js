...
function findOptimalTime(possibleTimes, invitedUsers, duration) {
  ...
  for (let perI = 0; perI < possibleTimes.length; perI++) {
    //loops over all trial times
    let trialI = 0;
    while (true) {
      ...
      if (errorRate == optimalTimes[0][2]) {
        //if error rate is equivalent, add the times
        optimalTimes.push([trialStart, trialEnd, errorRate]);
      } else if (errorRate < optimalTimes[0][2]) {
        //if error rate is lower, clear the array and add the times
        optimalTimes = [];
        optimalTimes.push([trialStart, trialEnd, errorRate]);
      }
...
function findOverlap(trialStart, trialEnd, invitedUsers) {
  ...
  for (let userI = 0; userI < invitedUsers.length; userI++) {
    ...
    for (let eventI = 0; eventI < userEvents.length; eventI++) {
      //iterates through events
      const event = DBUtility.eventObject(userEvents[eventI]);

      if (
        (event.startTime < trialStart && trialStart < event.endTime) ||
        (event.startTime < trialEnd && trialEnd < event.endTime)
      ) {
        //if the trial time starts/ends during the tested event
        errorRate++;
      } else if (
        (trialStart < event.startTime && event.startTime < trialEnd) ||
        (trialStart < event.endTime && event.endTime < trialEnd)
      ) {
        //if the event lies in the trial period
        errorRate++;
      } else if (trialStart == event.startTime || trialEnd == event.endTime) {
        //if the event is exactly the same time
        errorRate++;
